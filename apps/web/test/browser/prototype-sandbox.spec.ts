...
test.describe('Prototype Sandbox Tracer (Phase 0 Spike B)', () => {
  ...
  test('7. Enforces storage isolation (sandboxed code cannot read host cookies, localStorage, or sessionStorage)', async ({ page }) => {
    ...
    await expect(iframe.getByTestId('session-storage-probe-result')).toContainText('ISOLATION ENFORCED');
  });

  test('8. Enforces multi-vector network and navigation isolation across all probe vectors', async ({ page }) => {
    ...
    await expect(iframe.getByTestId('probe-fetch-status')).toHaveText('BLOCKED', { timeout: 10000 });
    await expect(iframe.getByTestId('probe-xhr-status')).toHaveText('BLOCKED', { timeout: 10000 });
    await expect(iframe.getByTestId('probe-beacon-status')).toHaveText('BLOCKED', { timeout: 10000 });
    await expect(iframe.getByTestId('probe-image-status')).toHaveText('BLOCKED', { timeout: 10000 });
    await expect(iframe.getByTestId('probe-script-status')).toHaveText('BLOCKED', { timeout: 10000 });
    await expect(iframe.getByTestId('probe-topnav-status')).toHaveText('BLOCKED', { timeout: 10000 });

    // Verify overall composite exfiltration boundary
    await expect(iframe.getByTestId('network-isolation-status')).toHaveText('ALL_EXFILTRATION_BLOCKED');
  });

  test('9. Terminates synchronous infinite loops via AST compiler guard without freezing host', async ({ page }) => {
  ...
  test('10. Recovers from asynchronous component hangs via host execution timeout and iframe teardown', async ({ page }) => {
