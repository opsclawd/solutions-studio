...
export const NAMING_ALIGNMENT_INSTRUCTIONS = [
  ...
  '- Sub-Resource and Entity Boundary Constraints:',
  ...
  '  - When requirements describe a business rule, state transition, or verification (such as state or lifecycle checks) that does NOT have a dedicated backing child table in the relational schema, model it strictly through:',
  '    (a) Parent resource lifecycle operations (e.g. "PATCH /<parents>/{id}" updating status or lifecycle columns, or action operations such as "POST /<parents>/{id}/<action>" matching recognized action verbs) using ONLY columns defined on the parent table, OR',
  '    (b) Sub-resource views whose request and response properties are strictly grounded in existing columns of the parent table, without inventing unbacked fields (e.g. do NOT invent separate identifiers, external transaction codes, or foreign attributes not present in the parent table schema). Parent-table views are allowed ONLY when every exposed property is grounded in parent columns.',
